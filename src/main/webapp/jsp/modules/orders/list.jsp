...
<body>
	  ...
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <h5 class="modal-title">指派骑手</h5>
	        <button type="button" class="close" data-dismiss="modal"><span>&times;</span></button>
	      </div>
	      <div class="modal-body">
	        <div class="form-group">
	          <label>选择骑手（空闲状态）</label>
	          <select id="assignQishouSelect" class="form-control">
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var qishounameCe = document.createElement('td');
			qishounameCe.innerHTML = item.qishouname || '';
			row.appendChild(qishounameCe);
			
			var qishouphoneCe = document.createElement('td');
			qishouphoneCe.innerHTML = item.qishouphone || '';
			row.appendChild(qishouphoneCe);
			
			var peisongstatusCe = document.createElement('td');
			var psColor = item.peisongstatus == '已送达' ? 'badge-success' : item.peisongstatus == '配送中' ? 'badge-warning' : 'badge-secondary';
			peisongstatusCe.innerHTML = '<span class="badge ' + psColor + '">' + (item.peisongstatus || '待分配') + '</span>';
			row.appendChild(peisongstatusCe);
			...
			if(orderStatus=="已支付") {
				...
				assignBtn.setAttribute("class","btn btn-info btn-sm");
				assignBtn.setAttribute("onclick","openAssignModal(" + item.id + ")");
				assignBtn.innerHTML = "指派骑手";
	...
	// 指派骑手
	var currentAssignOrderId = null;
	function openAssignModal(orderId) {
		currentAssignOrderId = orderId;
		$('#assignQishouSelect').empty().append('<option value="">-- 请选择骑手 --</option>');
		http("qishou/page","GET",{
			...
			if(res.code == 0 && res.data.list){
				res.data.list.forEach(function(q){
					var option = document.createElement('option');
					option.value = q.id;
					option.text = (q.xingming || q.username) + ' (' + (q.phone || '') + ')';
					$('#assignQishouSelect').append(option);
				});
			}
		});
	...
	function confirmAssign() {
		var qishouid = $('#assignQishouSelect').val();
		if(!qishouid){ alert('请选择骑手'); return; }
		httpJson("orders/assignQishou","POST",{
